#include "minmax.h"
#include <immintrin.h>


void minmax_sse(const unsigned char* v, int n, unsigned char* pmin, unsigned char* pmax){
    int i;
    unsigned char min[16];
    unsigned char max[16];
    __m128i vpmax = _mm_set1_epi8(0x00);
    __m128i vpmin = _mm_set1_epi8(0xFF);
    __m128i vv;
    *pmax = 0x00; // minimo valore unsigned char
    *pmin = 0xFF; // massimo valore unsigned char
    for (i=0; i+15 < n; i+=16) {
		vv = _mm_loadu_si128((const __m128i*)(v+i));
		vpmin = _mm_min_epu8(vpmin, vv);
		vpmax = _mm_max_epu8(vpmax, vv);
    }

    for(;i<n;i++) {
		if (v[i] > *pmax) *pmax = v[i];
        if (v[i] < *pmin) *pmin = v[i];
	}
	
	_mm_storeu_si128((__m128i*)max, vpmax);
    _mm_storeu_si128((__m128i*)min, vpmin);
    
     for (i=0; i<16; ++i) {
        if (min[i] < *pmin) *pmin = min[i];        
        if (max[i] > *pmax) *pmax = max[i];
    }
}


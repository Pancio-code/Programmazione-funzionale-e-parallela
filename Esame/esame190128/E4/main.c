...
//  Last changed:   $Date: 2019/01/27 15:00:07 $
...
void bw_host(unsigned char* in, unsigned char* out, 
    ...
    for (y=0; y<h; y++)
        for (x=0; x<w; x++)
            out[y*w+x] = in[y*w+x] < threshold ? 0 : 255;
...
void run_test(unsigned char* in, unsigned char* out, int h, int w,
              unsigned char threshold, clut_device* dev, char* msg) {
    ...
    bw_host(in, out_ok, h, w, threshold, &t);

    // compute darken on device
    bw(in, out, h, w, threshold, &td, dev);
...
int main(int argc, char** argv) {
    ...
    unsigned char   threshold;          // bw threshold
    ...
    // run test
    run_test(in, out, h, w, threshold,
             &dev, "Running test");

    // build pathname of output image file
    sprintf(out_path, "%s%s-bw%s", 

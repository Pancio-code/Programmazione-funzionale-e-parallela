...
int diff(unsigned char* a, unsigned char* b, int w, int h) {
    int x, y, err = 0;
    for (x=0; x<w; x++)
        for (y=0; y<h; y++)
...
int run_test(unsigned char* in1, int w1, int h1,
             unsigned char* in2, int w2, int h2,
             unsigned char** out, int* ow, int* oh,
    ...
    err = diff(*out, out_ok, ow_ok, oh_ok);
    ...
    if (*ow == ow_ok && *oh == oh_ok && err == 0)
        score = 10;
    else if (err < 0.1*ow_ok*oh_ok)
        score = 8;
...
int main(int argc, char** argv) {
    ...
    // run test
    score = run_test(in1, w1, h1, in2, w2, h2, &out, &ow, &oh, &dev, "Running test");

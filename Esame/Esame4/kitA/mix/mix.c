...
int       err;      // error code
    ...
    cl_mem    din1;      // input matrix on device
    ...
    *out = malloc((*oh)*(*ow)*sizeof(unsigned char));
    ...
    if (!din1) clut_panic("failed to allocate input1 matrix on device memory");
    
    // allocate input matrix on device as a copy of input matrix on host
    din2 = clCreateBuffer(dev->context, 
                         CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
                         h2*w2*sizeof(unsigned char), in2, NULL);
    if (!din2) clut_panic("failed to allocate input2 matrix on device memory");

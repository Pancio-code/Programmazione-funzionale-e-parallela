...
void scale2x_host(unsigned char* in, int w, int h,
    ...
    for (x=0; x<*ow; x++)
        for (y=0; y<*oh; y++) {
            unsigned c = (
                 ...
                 in[IDX(2*x+1, 2*y,   w)] +
                 in[IDX(2*x,   2*y+1, w)] +
                 in[IDX(2*x+1, 2*y+1, w)]
            ) >> 2;
            (*out)[IDX(x, y, *ow)] = (unsigned char)c;
        }
...
void run_test(unsigned char* in, int w, int h,
    ...
    scale2x_host(in, w, h, &out_ok, &ow_ok, &oh_ok, &t);

    // compute enlarge on device
    scale2x(in, w, h, out, ow, oh, dev, &td);
